...
# Bundled synthetic sample telemetry, generated by src/log_generator.py into
# a single queryable SQLite database rather than one JSONL file per source
# (see write_dataset()'s docstring for the schema).
SAMPLE_LOG_DB = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "sample_logs.db")

LOG_SOURCES = {
    "auth_events": {"label": "Identity / Auth Logs"},
    "firewall_events": {"label": "Firewall / Proxy Logs"},
    "edr_alerts": {"label": "EDR Endpoint Alerts"},
    "cloud_audit_events": {"label": "Cloud Audit Logs"},
}
